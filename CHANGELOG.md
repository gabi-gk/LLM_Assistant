...
26.04.2026
CREATE check_gpu.py - verify CUDA and VRAM before running
CREATE download_model.py - one-time download of Qwen3-8B weights to local disk
CREATE chat.py - basic chat loop, Qwen3-8B quantized to 4-bit nf4
...
27.04.2026
CREATE rag.py - ChromaDB vector database for long term external memory
...
CREATE tools/shell.py - shell command execution with risk classification
CREATE tools/notifications.py - immediate, scheduled and persistent reminders
CREATE agent/registry.py - tool registry and descriptions for model
CREATE agent/loop.py - agent loop with tool call parsing and execution
...
03.05.2026
CREATE tray/window.py - tkinter terminal-style dark chat window
CREATE tray/app.py - system tray icon, Alt+Space hotkey, app lifecycle
CREATE run.py - GUI entry point (main.py kept for terminal/debug use)
CREATE core/search.py - shared RAG search logic extracted from main.py
...
CREATE tools/knowledge.py - RAG is now controlled by the model
EDIT agent/registry.py - added tools to search the knowledge base
EDIT tray/app.py - removed automatic RAG

09.05.2026
CREATE tools/windows.py - window listing, switching, minimizing, maximizing
CREATE tools/apps/system.py - open and find installed applications
CREATE tools/apps/discord_bot.py - Discord bot with per-user conversation history
EDIT tray/app.py - Discord bot starts on startup with shared model
CREATE tools/apps/discord_bot.py - added clear and improved logging
...
CREATE tools/browser.py - web search via SearXNG and DuckDuckGo

13.05.2026
CREATE finetune_dpo.py - DPO training pipeline (adapted from MEng dissertation project)
CREATE prepare_training_data.py - generate synthetic fine-tuning pairs 
...
CREATE data/information/marvin_self.md - Marvin's identity and self knowledge file
...
CREATE log_review.py - to quickly sorft through logs and pick up the training data

20.05.2026
CREATE training/prepare_SFT_data.py - generated SFT training data
CREATE training/prepare_DPO_data.py - separated prepare_training_data.py to be usable by both methods
CREATE finetune_sft.py - copied the DPO framework and adjusted it for SFT
MOVE finetune_dpo.py to training/
CREATE log_review.py - simple log clear up script
...
30.05.2026
CREATE merge.py - mergining the training models into one final model
CREATE training_combined.py - run the full training pipeline at once
...
08.06.2026
EDIT core/ultils.py - add clear markers for self injection start, end and acknowledgement
EDIT core/history.py - remove the self model from compaction and re-inject it afterwards
EDIT core/loop.py - add an error case for missing arguments during a tool call
